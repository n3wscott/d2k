...
<g transform="rotate({{ .rotate1 }} 80 110)">
...
<g transform="rotate({{ .rotate2 }} 80 220) translate(6 100) scale(.25 0.25)">
...
func main() {
	...
	for i, r := range rotations {
		f, err := os.Create(fmt.Sprintf("popcorn-matt-%02d.svg", i))
		if err != nil {
			fmt.Printf("can't party :( %s\n\n", err)
			continue
		}
		...
		if err := party.Execute(f, map[string]string{
			"rotate1": fmt.Sprintf("%d", r),
			"rotate2": fmt.Sprintf("%d", -1*r/2),
